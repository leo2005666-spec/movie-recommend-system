...
- **登录/注册右侧流动海报**：`AuthFilmflow` 合并 **两页 `/movies` 列表**（不同排序）后按 **本地 id、tmdb_id、规范化标题、封面 URL、海报路径** 去重；静态兜底列表按 **路径 token** 去重。**首页/列表电影卡片**：`.movie-card__cover-wrap` 固定 **2:3** 比例并裁剪，封面图 **`object-fit: cover`** 填满，避免同一行海报高度不齐。
