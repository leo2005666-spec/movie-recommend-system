...
| 用户信息管理 | 查看、修改个人信息，API支持增删查改；**前台与接口不再使用「昵称」**，展示名即 **用户名**（库内仍保留 `nickname` 列，每次数据库初始化会执行 `nickname = username` 与历史数据对齐） | ✓ |
...
| 个性化推荐 | 根据推荐算法展示个性化列表 | ✓ |
| 人群口味推荐 | 学生党、上班族、家庭亲子快捷标签筛选（情侣约会/资深影迷已从前端入口隐藏） | ✓ |
...
- `/api/recommend/tastes` - 人群口味预设列表（当前前端默认展示：学生党、上班族、家庭亲子）
...
系统内置人群口味预设，当前前端默认展示三类（学生党、上班族、家庭亲子）：
...
| 人群口味推荐 | `tasteType` 参数（前端默认 `student/worker/family`），首页/推荐页/影视库快捷标签 | ✓ |
...
- **登录/注册右侧海报背景**：`AuthFilmflow` 采用 **3×4 静态宫格**，同屏不重复；素材池先按 **本地 id / tmdb_id / 标题 / 封面路径** 去重，再渲染。刷新频率改为**低频定时整体刷新**（默认 **1 小时**，可用 `VITE_AUTH_FILMFLOW_REFRESH_MS=86400000` 调成 **1 天**），不再每几秒切图。**首页/列表电影卡片**：`.movie-card__cover-wrap` 用 **`height:0` + `padding-bottom:150%`** 固定 **2:3** 占位，封面 **`position:absolute` + `object-fit:cover`** 填满；并对首页横滑卡片 `.info` 设固定最小高度、标题锁定两行，避免“看起来封面长短不一”。
- **影片提名页修复（站内展示）**：点击影片详情中的 **AWARDS/提名** 现在只在本站路由 `/movies/:id/awards` 展示；奖项列表组件移除了组织/典礼/类别等外部跳转链接，未入库影片海报仅展示不外跳；后端对奖项映射查询增加数组兜底，避免 `rows.forEach is not a function`。
- **奖项区域交互与性能**：奖项卡的 **Shared with** 人员名单默认收起，点击“展开名单”后再渲染整列头像与姓名；影片详情页不再内嵌加载完整奖项列表，改为仅展示站内 AWARDS 入口，点进 `/movies/:id/awards` 才请求并渲染详情，减少详情页首屏请求与图片压力。
- **系列合集页跳转（站内优先）**：在 `/movies/collection/:tmdbCollectionId` 点击系列影片时，若已入库则直接进入本站 `/movies/:id`；若未入库会先调用 `POST /api/movies/from-tmdb/:tmdbId` 自动导入，再跳转到本站详情，不再外跳 TMDB。
- **演员阵容与片单交互**：影视详情与剧集详情的演员区仅展示**有头像**的演员（无图不显示占位圆块）；详情页顶部新增可交互的 **片单 / 收藏 / 待看** 按钮，其中片单与待看走 `user_movie_shelves`（`/api/shelves/*`），用户可在 `/shelves` 查看和管理自己的片单、待看。
- **个性推荐差异化与补库**：推荐回退逻辑新增用户行为信号（评分≥4、收藏、片单、待看、点击埋点）联合建模；当登录用户推荐为空时，前端会触发一次 TMDB 热门补库（`/api/tmdb/lists` + `/api/movies/from-tmdb/:tmdbId`）再刷新推荐，减少“不同用户看起来都一样”与“库内缺片”问题。
- **站内讨论论坛（知乎式话题）**：`/forum` 顶部先展示「电影 / 演员 / 求推荐 / 影评讨论 / 片单」等**大话题入口**，点话题后以下方信息流方式展示该话题下帖子；`/forum/:id` 为帖子详情与楼中楼回复。首页「讨论论坛」区块会展示话题标签 + 最新讨论。管理员仍可一键生成“虚拟讨论”（仅管理员可见按钮），用于冷启动撑场面。
- **论坛管理能力补充**：管理员在 `/forum/:id` 可直接**删除帖子**，也可对任意楼层执行**删除回复**（会连带删除该回复下的子回复，避免出现无父节点的楼层）。
- **主题切换按钮**：顶栏已移除暗夜模式切换按钮，当前前台固定为统一主题展示。
- **影视库分页**：列表接口与前端默认 **每页 15 条**，便于 5 列网格 **3 行铺满**。
- **演员页**：影视详情「演员阵容」中点击演员进入 **`/actors/:tmdbPersonId`**。布局为 **TMDB 式左栏资料 + 右栏片单**，主内容区 **`actor-page--fullbleed`** 与影视库同档 **加宽（约 1680px）**、**横向铺满**（避免窄容器居中）；**系统无衬线字体栈**（含 Noto Sans SC / 苹方 / 微软雅黑）。侧栏：头像、知名领域、参与作品数、性别/生日/出生地/又名、资料完整度、TMDB/IMDb/官网链接。主区：生平简介、**知名作品**横滑海报轨、其下为 **奖项与提名** 入口：**深色 AWARDS 条** 点击进入 **`/actors/:tmdbPersonId/awards`** 独立页（顶栏小头像 + 姓名 +「← 返回主页面」+ **白底 TMDB 式分组列表**：颁奖机构青蓝标题 `#01b4e4`、左侧机构 Logo、右侧卡片含典礼名、绿/灰 **获奖·提名** 角标、类别、影片海报、Shared with 头像行、年份）；数据来自 **`GET /api/actors/:tmdbPersonId/awards`**；图片经 **`GET /api/proxy-img?u=`**，后端允许 **`image.tmdb.org`** 与 **`media.themoviedb.org`**（后端用 **cheerio** 解析 TMDB 官网人物奖项页 HTML，与官网一致；若 TMDB 改版可能需跟进选择器）。再下为 **全部作品**表格式列表（年份｜圆点｜标题 + 饰演），支持 **全部 / 仅本站已收录** 与 **排序**；已入库链本站详情，未入库链 TMDB。需后端配置 **TMDB_API_KEY**。
