...
详见 [DEPLOY.md](./DEPLOY.md)，可将项目部署到 **Vercel（前端）+ Render（后端）**，生成在线链接供他人访问。
...
### 用户画像迁移（收集性别、年龄用于推荐）
若数据库已存在，需执行：`node backend/scripts/migrate-user-demographics.js`

### 注册与账号
- **注册**：用户名+密码即可注册，支持选填性别、年龄、昵称。无邮箱、无验证码
- **老用户补充性别/年龄**：早期注册用户若缺少性别或年龄，可到「个人信息」页补充，用于提升个性化推荐效果
