...
router.post('/', (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO favorites (user_id, movie_id) VALUES (?, ?)').run(req.user.id, movieId);
    logActivity(req, 'FAVORITE', 'movie', movieId, movie.title);
...
router.delete('/:movieId', (req, res) => {
  ...
  db.prepare('DELETE FROM favorites WHERE user_id = ? AND movie_id = ?').run(req.user.id, movieId);
  logActivity(req, 'UNFAVORITE', 'movie', movieId, '取消收藏');
