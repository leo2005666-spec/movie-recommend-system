...
router.get('/me', authMiddleware, asyncHandler(async (req, res) => {
  ...
  const total = (await db.prepare('SELECT COUNT(*) as n FROM activity_logs WHERE user_id = ?').get(req.user.id)).n;
...
router.get('/', authMiddleware, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const total = (await db.prepare(`SELECT COUNT(*) as n FROM activity_logs ${where}`)
    .get(...(userId ? [userId] : []))).n;
