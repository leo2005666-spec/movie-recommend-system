...
router.get('/', optionalAuth, (req, res) => {
  ...
  const total = db.prepare('SELECT COUNT(*) as n FROM qa_posts WHERE parent_id IS NULL').get().n;
...
router.post('/', authMiddleware, [
  ...
  try {
    db.prepare('INSERT INTO qa_posts (user_id, title, content, is_answer) VALUES (?, ?, ?, 0)')
      .run(req.user.id, title.trim(), content.trim());
    const row = db.prepare('SELECT last_insert_rowid() as id').get();
    logActivity(req, 'QA_ASK', 'qa', row.id, title);
...
router.post('/:id/answer', authMiddleware, [
  ...
  const question = db.prepare('SELECT id FROM qa_posts WHERE id = ? AND parent_id IS NULL').get(questionId);
  if (!question) return res.status(404).json({ code: 404, message: '问题不存在' });
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
  ...
  try {
    db.prepare('INSERT INTO qa_posts (user_id, title, content, is_answer, parent_id) VALUES (?, ?, ?, 1, ?)')
      .run(req.user.id, '', content.trim(), questionId);
    logActivity(req, 'QA_ANSWER', 'qa', questionId, '回答问题');
